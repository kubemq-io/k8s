	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				...
				StringVariables: map[string]string{"LICENSE_KEY_DATA": `-------License---------qweqwweqweqweq`},
			},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := SecretConfig{
				...
				StringVariables: tt.fields.StringVariables,
			}
			...
			if tt.wantErr {
				...
				for key, value := range tt.fields.StringVariables {
